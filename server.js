  ...
  fs.readFile(filePath, "utf8", (err, fileData) => {
    ...
    if (fileData) {
      try {
        ...
        return res
          .status(500)
          .json({ message: "Erro ao processar os dados existentes." });
      }
    }
...
// Rota para obter dados de compras
app.get("/purchases", (req, res) => {
    ...
    readJsonFile(filePath)
      ...
      .catch(error => {
        console.error("Erro ao ler o arquivo de compras:", error);
        res.status(500).json({ error: "Erro ao ler o arquivo de compras" });
      });
  });  
...
app.post("/retiradas", (req, res) => {
  ...
  fs.readFile(filePathRetiradas, "utf8", (err, fileData) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ message: "Erro ao ler o arquivo de retiradas." });
    }
    ...
    fs.writeFile(
      filePathRetiradas,
      JSON.stringify(retiradas, null, 2),
      (err) => {
        if (err) {
          console.error("Erro ao salvar a retirada:", err);
          return res
            .status(500)
            .json({ message: "Erro ao salvar a retirada." });
        }

        // Atualizar o arquivo de materiais
        const filePathMaterials = path.join(
          __dirname,
          "public",
          "materials.json"
        );
        ...
        fs.readFile(filePathMaterials, "utf8", (err, materialsData) => {
          if (err) {
            ...
            return res
              .status(500)
              .json({ message: "Erro ao ler o arquivo de materiais." });
          }
          ...
          try {
            ...
            console.error(
              "Erro ao processar os dados JSON dos materiais:",
              parseError
            );
            return res
              .status(500)
              .json({ message: "Erro ao processar os dados dos materiais." });
          }
          ...
          fs.writeFile(
            ...
            (err) => {
              if (err) {
                console.error(
                  "Erro ao atualizar a quantidade de materiais:",
                  err
                );
                return res.status(500).json({
                  message: "Erro ao atualizar a quantidade de materiais.",
                });
              }
...
app.get("/retiradas", (req, res) => {
  ...
  fs.readFile(filePathRetiradas, "utf8", (err, data) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ error: "Erro ao ler o arquivo de retiradas" });
    }
    try {
      ...
      console.error(
        "Erro ao processar os dados JSON das retiradas:",
        parseError
      );
      res
        .status(500)
        .json({ error: "Erro ao processar os dados das retiradas" });
    }
  });
});
